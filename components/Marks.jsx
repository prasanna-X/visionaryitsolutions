...
export function CloverMark({ size = 40, fill = C.ink, stroke = "none" }) {
  return (
...
export function CloverBullet({ color = C.accent, size = 9 }) {
  return (
...
/* Full circular badge — now a static PNG emblem instead of hand-drawn SVG.
   Drop the exported artwork at /public/images/badge.png (square, ideally
   2x the largest rendered size for crisp retina display). */
export function Badge({ size = 220, src = "/images/vits_logo.png" }) {
  return (
    <Image
      src={src}
...
export function CircuitField({ opacity = 0.06 }) {
  return (
    <svg
